...
async function resolveIdentity(gateway: AuthorizationGateway): Promise<VerifiedIdentity> {
  ...
  if (!claims.success) {
    throw new AuthorizationError("Verified claims are invalid");
  }
  ...
  if (roles.length !== rolesResult.roles.length) {
    throw new AuthorizationError("Account roles are invalid");
  }
...
export function authorize(gateway: AuthorizationGateway) {
  return {
    ...
    async requireAdmin(options: { requireMfa?: boolean } = {}) {
      ...
      if (options.requireMfa && identity.aal !== "aal2") {
